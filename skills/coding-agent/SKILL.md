...
- **🔴 MUST KILL all coding agent processes after task completion** (private fork rule). Every `claude`, `gemini`, `codex`, `pi` process you spawned MUST be killed when your task ends — use `process action:list` → `process action:kill sessionId:{id}` for each running session, plus orphan cleanup: `ps aux | grep -E "claude.*--print|gemini.*-p|codex" | grep "/tmp/work-" | grep -v grep | awk '{print $2}' | xargs -r kill`. Failure causes memory leaks → system OOM → Gateway crash. Applies to ALL exit paths: success, failure, blocked, timeout.
